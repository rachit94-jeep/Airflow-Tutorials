...
@dag(
    dag_id='incremental_load_dag_4',
    start_date=datetime(2026, 7, 13, tz='Asia/Kolkata'),
    tags=['ETL', 'incremental'],    
    schedule=CronDataIntervalTimetable('@daily',timezone = 'Asia/Kolkata'),  # 1-day windows; change to timedelta(hours=1) for hourly loads
    catchup=True  # backfill missed windows — required for incremental ETL
)
def incremental_load_dag4():
