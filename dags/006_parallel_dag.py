...
def parallel_dag():
    ...
    @task
    def extract(**kwargs):
        # Push two separate datasets to XCom for downstream parallel tasks
        ti = kwargs['ti']
    ...
    # Runs as long as at least one upstream branch succeeded (tolerates a skipped branch)
    @task(trigger_rule='none_failed_min_one_success')
    def createDataframe(**kwargs):
        # Merge S3 (index) and Snowflake (names) data into a single DataFrame
        ti = kwargs['ti']
    ...
    # transform_s3 and transform_sf run in parallel after extract
